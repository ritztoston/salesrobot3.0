export const GET_ERRORS = 'GET_ERRORS';

export const CLEAR_ERRORS = 'CLEAR_ERRORS';
export const SET_LOADING_TRUE = 'SET_LOADING_TRUE';
export const SET_LOADING_FALSE = 'SET_LOADING_FALSE';
export const SET_MESSAGE = 'SET_MESSAGE';
export const SET_UNLOAD_MESSAGE = 'SET_UNLOAD_MESSAGE';

export const SET_G_LOADING_TRUE = 'SET_G_LOADING_TRUE';
export const SET_G_LOADING_FALSE = 'SET_G_LOADING_FALSE';

export const SET_CURRENT_USER = 'SET_CURRENT_USER';

export const GET_USERS = 'GET_USERS';
export const CLEAR_USERS = 'CLEAR_USERS';

export const GET_VERIFICATION_PAYLOAD = 'GET_VERIFICATION_PAYLOAD';
export const GET_TEMPLATE = 'GET_TEMPLATE';
export const GET_RSS_FEED_TEMPLATE = 'GET_RSS_FEED_TEMPLATE';
export const GET_RSS_DATA = 'GET_RSS_DATA';
export const GET_INDEX_DATA = 'GET_INDEX_DATA';
export const CLEAR_ACCOUNT_DATA = 'CLEAR_ACCOUNT_DATA';
export const CLEAR_CAMPAIGN_DATA = 'CLEAR_CAMPAIGN_DATA';
export const GET_SENT_DATA = 'GET_SENT_DATA';
export const GET_ACTIVE_DATA = 'GET_ACTIVE_DATA';
export const GET_DRAFT_DATA = 'GET_DRAFT_DATA';

export const SET_MESSAGE_SENT = 'SET_MESSAGE_SENT';
export const SET_CAMPAIGN_DETAIL_VIEW = 'SET_CAMPAIGN_DETAIL_VIEW';
